...
import (
	...
	"github.com/duolacloud/microbase/database/elasticsearch"
...
func getTenancy(config config.Config) (multitenancy.Tenancy, error) {
	client, err := elasticsearch.NewElasticSearchClient(config)
	if err != nil {
		logger.Fatal("数据库连接失败", err)
		return nil, err
	}

	tenancy := elasticsearch.NewElasticsearchTenancy(client, &EntityMap{})
